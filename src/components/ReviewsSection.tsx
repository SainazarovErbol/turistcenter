    ...
    <section id="reviews" className="py-20 bg-muted/30">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold text-foreground mb-3" style={{ fontFamily: "var(--font-heading)" }}>
            {t("title")}
          </h2>
            ...
            return (
              <div key={review.id} className="break-inside-avoid rounded-xl border border-border bg-card p-5 space-y-3">
                ...
                <div className="flex items-center gap-0.5">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <Star key={i} className={`h-3.5 w-3.5 ${i <= review.rating ? "fill-yellow-400 text-yellow-400" : "fill-muted text-muted"}`} />
                  ))}
                </div>
                <p className="text-sm text-muted-foreground leading-relaxed">{review.text}</p>
                {place && (
                  <Link href={`/places/${place.id}`} className="inline-block text-xs text-primary hover:underline">
                    📍 {placeName}
                  </Link>
                )}
                <div className="flex items-center gap-2.5 pt-2 border-t border-border">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{getLocalizedCountry(review.authorCountry, locale)}</p>
                  </div>
                </div>
              </div>
            );
